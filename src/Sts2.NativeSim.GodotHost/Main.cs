...
public partial class Main : Node
{
    ...
    {
        if (OperatingSystem.IsWindows()) SetErrorMode(ErrorMode);
        int exitCode = 1;
        try
        {
            ...
            string? configuredAssembly = System.Environment.GetEnvironmentVariable("STS2_ASSEMBLY");
            string assemblyPath = args.FirstOrDefault(x => !x.StartsWith("--", StringComparison.Ordinal))
                ?? configuredAssembly
                ?? throw new ArgumentException("Pass the sts2.dll path after -- or set STS2_ASSEMBLY.");
            ...
            if (!ProjectSettings.LoadResourcePack(pckPath, false)) throw new InvalidOperationException($"Could not mount {pckPath}");
            ...
            else
            {
                ...
                NativeFeasibilityReport report = await new NativeFeasibilityProbe().RunAsync(assemblyPath, iterations, true);
                Console.WriteLine("NATIVE_SIM_REPORT_BEGIN"); Console.WriteLine(JsonSerializer.Serialize(report, new JsonSerializerOptions { WriteIndented = true })); Console.WriteLine("NATIVE_SIM_REPORT_END");
                exitCode = report.AllRequiredStagesPassed ? 0 : 1;
            }
        }
        catch (Exception exception) { Console.Error.WriteLine($"NATIVE_SIM_HOST_FAILURE\n{exception}"); }
        finally { GetTree().Quit(exitCode); }
    }

    private static async Task<int> RunOptionChoiceAcceptanceAsync(string assemblyPath, string pckPath)
    {
        using PersistentNativeCombatEnvironment environment = new(assemblyPath, pckPath);
    ...
    private static async Task<int> RunTraceExporterSmokeAsync(string assemblyPath, string pckPath, string exporterPath)
    {
        string traceDirectory = Path.Combine(OS.GetUserDataDir(), "native_sim_traces");
        ...
        System.Environment.SetEnvironmentVariable("STS2_NATIVE_TRACE", "1");
        System.Environment.SetEnvironmentVariable("STS2_NATIVE_TRACE_SOURCE", "simulator_self_smoke");
        using PersistentNativeCombatEnvironment environment = new(assemblyPath, pckPath);
        Assembly exporter = (AssemblyLoadContext.GetLoadContext(typeof(Main).Assembly) ?? AssemblyLoadContext.Default).LoadFromAssemblyPath(Path.GetFullPath(exporterPath));
        ...
        System.Environment.SetEnvironmentVariable("STS2_NATIVE_TRACE", null);
        System.Environment.SetEnvironmentVariable("STS2_NATIVE_TRACE_SOURCE", null);
        ...
        while (await Console.In.ReadLineAsync() is { } line)
        {
            ...
            try
            {
                ...
                object? result = request.Method switch
                {
                    ...
                    "custom_reward_reset" => await runCoordinator.CustomRewardResetAsync(Read<CustomRewardResetRequest>(request.Parameters)),
                    "rest_reset" => runCoordinator.RestReset(Read<ResetRequest>(request.Parameters)),
