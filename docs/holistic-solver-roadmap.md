...
An isolated audit of `zhiyue/sts2-rl-agent` revision `1b7e7ce35e608722650763938c153ea8bc370333` is recorded in `artifacts/shadow-simulator-audit.json`. Its 4,609-test suite passed with one skip, and the local random-action benchmark measured 3,490 steps/second and 140 combats/second, but a three-Strike Bygone Effigy slice matched native energy, piles, Slow scaling (6/6/7 damage), and the complete sleep/wake turn only after explicitly importing `sts2_env.powers`; on the apparent production trainer path Slow was absent and the third Strike incorrectly dealt 6. The external simulator therefore remains an optional, unlicensed, non-authoritative candidate generator: it is not a label source or product dependency. (The former `docs/shadow-simulator-audit.md` was retired in the repository cleanup commit `d6371e0`; this paragraph and the constraint above are its durable conclusion.)
...
- **Phase 1: authoritative simulation & 20-worker farm PROVEN (GO).** Shipped mechanics run directly in isolated, presentation-suppressed .NET 9 workers with sustained 1,514+ decisions/second throughput and 100% zero-crash stability. Mechanical fidelity is claimed only as exact differential replay on the declared, build-pinned checkpoint matrix; global certification remains false.
...
- **Strategic Direction: STS1 prior-art architecture integration.** The project adopts seven foundational research principles derived from mature STS1 systems (Spire Pilot, AlphaStS, sts_lightspeed, bottled_ai, sts-rl-agent). The principles are fixed in `docs/architectural-guardrails.md` §0; the external comparative analysis behind them is not maintained in this repository. Search is recognized as a permanent first-class runtime component (PUCT / hybrid beam), categorical HP/death-risk heads replace scalar expectations, and training transitions to iterative DAgger loops.
...
The program currently executing toward that path is the first-combat corpus program (`docs/first-combat-scene-generation-plan.md`): its E1–E4 units are implemented in the tree, and the E5 FullApp golden differential and E6 corpus farm gates are outstanding. Dated status and the ordered milestone list are owned by `docs/project-status-and-review-guide.md`.
