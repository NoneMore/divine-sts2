...
# tree, but the documents that describe the rule must be able to quote it — the B1 finding
# in docs/architecture-review.md was itself a match, which is why markdown is exempt from
# the path scan. A leaked credential is a leak in prose too, so the token scan still covers
# every tracked file.
